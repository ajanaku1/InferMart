...
Production hardening is also out of scope, with one closed exception: the seller now validates untrusted audio input. A peer sending a malformed `f32le` buffer directly used to be able to wedge the seller's Whisper model until restart, the queue-poisoning bug filed as [#3221](https://github.com/tetherto/qvac/issues/3221). Until that lands upstream, the metering plugin rejects any buffer that is not a whole number of 32-bit samples before it reaches the model ([`packages/shared/audio-guard.js`](packages/shared/audio-guard.js)), failing only the offending request.
